        ...
        case 'CLIENTE':{
            ...
            fetch(url_form)
             ...
             .then( form => {
                   ...
                   fetch(`http://127.0.0.1:3000/personas/${_local_id}`, requestOptions)
    ...
    for ( var i = 0; i < elementosFormulario.length ; i++  ){
        ...
        if ( elementosFormulario[i].nodeName === 'INPUT' ) {
            switch( elementosFormulario[i].name ) {
                ...
                case 'accion': {
                    persona.accion = elementosFormulario[i].value;
                    break;
                }                             
            }
        }
...
function guardaMascota( _btn_ ) {
    ...
    for ( var i = 0; i < elementosFormulario.length ; i++  ){
        ...
        if ( elementosFormulario[i].nodeName === 'INPUT' || elementosFormulario[i].nodeName === 'SELECT'    ) {
            switch( elementosFormulario[i].name ) {
                case 'nombre_mascota': {
                    mascota.nombre_mascota = elementosFormulario[i].value;
                    break;
                }
                case 'tipo_mascota': {
                    mascota.tipo_mascota = elementosFormulario[i].value;
                    break;
                }
                case 'acompanante': {
                    mascota.acompanante = elementosFormulario[i].value;
                    break;
                }                                
            }
        }
 ...
 * 
 */
function dibujarTabla( _form_, _area_trabajo_, personas){
    if ( personas && personas.length > 0 ) {
        for ( var i = 0 ; i < personas.length; i++ ){
                         var fila =  `<tr><td>${personas[i].id}</td>
                                            <td>${personas[i].nombre}</td>
                                            <td>${personas[i].apellido}</td>
                                            <td>${personas[i].celular}</td>
                                            <td>
